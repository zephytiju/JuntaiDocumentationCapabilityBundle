...
describe("bounded Console SDK compatibility", () => {
  it.each(["2.1.0", "2.2.0"])("accepts supported SDK %s", (sdkVersion) => {
    expect(() => assertDeterministicConsoleModule(module, { compatibilityTarget: { consoleVersion: "1.1.0", sdkVersion, reactVersion: "19.2.7", typescriptSdkVersion: "3.10.0" } })).not.toThrow();
  });
  it.each(["2.0.0", "2.2.1", "3.0.0"])("rejects unsupported SDK %s", (sdkVersion) => {
    expect(() => assertDeterministicConsoleModule(module, { compatibilityTarget: { consoleVersion: "1.1.0", sdkVersion, reactVersion: "19.2.7", typescriptSdkVersion: "3.10.0" } })).toThrow();
  });
